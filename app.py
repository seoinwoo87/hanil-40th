...
def load_all_data():
    try:
        ...
        if not df_master.empty and '고유번호' in df_master.columns:
            ...
            return [d.assign(고유번호=d.get('표시식별', '')) for d in [df_scores, df_mock, df_ref, df_act, df_counsel]]
...
# ==========================================
# 5. 사이드바 구성 (프라이버시 세팅 및 NameError 해결)
# ==========================================
...
with st.sidebar:
    ...
    # [수정] c_students -> class_students 로 오타 수정 완료
            ...
            if exam == "학기말":
                ...
                for _, row in filtered.iterrows():
                    ...
                    plot_data.append({
                        '과목': row['과목'], 
                        '점수': round(my_score, 2), 
                        '중위값': round(all_exam.median(), 2) if not all_exam.empty else 0, 
                        '백분위': round(perc, 2)
                    })
    ...
    with t2:
        ...
        if not f_df.empty and u_col in f_df.columns:
            ...
            sel_rows = st.data_editor(
                f_df[['학기', '과목', '점수', g_col, '성취도', u_col, '9등급(자동)']], 
